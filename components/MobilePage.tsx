...
function SectionIntro({ id, title, subtitle, dark = false }: { id: string; title: string; subtitle: string; dark?: boolean }) {
  return (
    <div id={id} className={`${dark ? "bg-[#111]" : "bg-[#f28a12]"} scroll-mt-24 px-5 pb-12 pt-32`}>
      <h2 className="max-w-full font-serif text-[clamp(70px,18vw,96px)] leading-[0.9] tracking-[-0.065em] text-white">{title}</h2>
      <p className={`mt-7 max-w-full text-[24px] leading-[1.25] ${dark ? "text-[#e7bd91]" : "text-[#15120d]"}`}>{subtitle}</p>
    </div>
  );
}
...
function CTA({ children, dark = false, onClick }: { children: ReactNode; dark?: boolean; onClick?: () => void }) {
  return (
    <button onClick={onClick} className={`flex min-h-[58px] w-full items-center justify-center rounded-[14px] px-6 text-[19px] font-medium ${dark ? "bg-[#111] text-white" : "border border-[#f28a12] text-[#f28a12]"}`}>
      <span>{children}</span>
...
function MobileHero() {
  return (
    <section id="m-hero" className="scroll-mt-24 bg-[#f28a12] px-5 pb-14 pt-32">
      <p className="text-[14px] font-bold uppercase leading-6 tracking-[0.16em] text-[#15120d]">Verkkosivut, jotka toimivat ja näyttävät hyvältä.</p>
      <h1 className="mt-8 max-w-full font-serif text-[clamp(58px,17vw,90px)] leading-[0.95] tracking-[-0.065em] text-white">Selkeät verkkosivut pienille yrityksille</h1>
      <p className="mt-8 text-[23px] leading-[1.35] text-[#15120d]">Teen moderneja ja mobiiliystävällisiä verkkosivuja yrityksille, jotka haluavat näyttää luotettavilta ja tehdä yhteydenotosta helppoa.</p>
      <p className="mt-7 text-[21px] leading-[1.4] text-[#15120d]/90">Ei turhaa monimutkaisuutta — vain siisti sivu, selkeä rakenne ja toimiva yhteydenotto.</p>
      <div className="mt-10 grid gap-4">
        <CTA dark onClick={() => goTo("m-yhteys")}>Pyydä tarjous</CTA>
        <button onClick={() => goTo("m-hinnat")} className="min-h-[58px] rounded-[14px] border border-[#15120d] text-[19px] font-medium text-[#15120d]">Katso hinnat</button>
      </div>
      ...
      <img src="/puuhaus-esimerkki.jpg" alt="Puuhaus-esimerkkisivu" loading="eager" decoding="async" className="mt-12 w-full rounded-[22px] shadow-[0_24px_70px_rgba(0,0,0,0.34)]" />
    </section>
  );
}
...
export default function MobilePage() {
  ...
  return (
    <main className="w-full max-w-full overflow-x-hidden bg-[#f28a12] text-[#f7f3ec] md:hidden">
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[80] bg-black/70 p-4 backdrop-blur-xl">
          <div className="rounded-[24px] border border-white/10 bg-[#111] p-6">
            <div className="flex items-center justify-between"><Logo /><button onClick={() => setMenuOpen(false)} className="grid h-12 w-12 place-items-center rounded-xl border border-white/10" aria-label="Sulje valikko"><X /></button></div>
            <div className="mt-8 grid">{navItems.map((item) => <button key={item.id} onClick={() => { setMenuOpen(false); goTo(item.id); }} className="border-b border-white/10 py-5 text-left font-serif text-4xl text-white">{item.label}</button>)}</div>
          </div>
        </div>
      )}
      ...
      <section className="bg-[#111] px-5 py-10"><div className="grid gap-5">{processSteps.map((step, index) => { const Icon = step.icon; return <DarkCard key={step.title} className="p-6 text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full border border-[#f28a12] text-2xl text-[#f28a12]">{index + 1}</div><Icon className="mx-auto mt-8 h-16 w-16 text-[#f28a12]" strokeWidth={1.45} /><h3 className="mt-8 font-serif text-[31px] tracking-[-0.04em] text-white">{step.title}</h3><div className="mx-auto mt-5 h-[2px] w-14 bg-[#f28a12]" /><p className="mt-6 text-[16px] leading-7 text-white/65">{step.text}</p></DarkCard>; })}</div><DarkCard className="mt-8 p-6"><div className="flex items-center gap-5"><Send className="h-14 w-14 rounded-full border border-[#f28a12] p-3 text-[#f28a12]" /><div><h3 className="font-serif text-3xl text-white">Valmis aloittamaan?</h3><p className="mt-1 text-white/65">Otan mielelläni vastaan uusia projekteja.</p></div></div><div className="mt-6"><CTA onClick={() => goTo("m-yhteys")}>Pyydä tarjous</CTA></div></DarkCard></section>

      <SectionIntro id="m-hinnat" title="Hinnat" subtitle="Selkeät paketit pienille yrityksille." />
      <section className="bg-[#f28a12] px-5 pb-12"><div className="grid gap-5">{pricing.map((plan) => <DarkCard key={plan.name} className={`p-7 ${plan.featured ? "border-[#f28a12]" : ""}`}>{plan.featured && <span className="mb-5 inline-block rounded-full bg-[#f28a12] px-5 py-2 text-xs font-bold uppercase text-[#15120d]">Suosituin</span>}<h3 className="font-serif text-[40px] text-white">{plan.name}</h3><p className="mt-5"><span className="mr-2 text-[#f28a12]">alk.</span><span className="font-serif text-[68px] leading-none text-[#f28a12]">{plan.price}</span></p><div className="mt-5 h-[2px] w-16 bg-[#f28a12]" /><ul className="mt-7 grid gap-4">{plan.features.map((feature) => <li key={feature} className="flex gap-3 text-white/80"><CheckCircle2 className="h-5 w-5 shrink-0 text-[#f28a12]" />{feature}</li>)}</ul></DarkCard>)}</div><p className="mt-7 text-center text-[#15120d]">Lopullinen hinta riippuu sivujen laajuudesta ja sisällöstä.</p><div className="mt-6"><CTA dark onClick={() => goTo("m-yhteys")}>Pyydä tarjous</CTA></div></section>

      <SectionIntro id="m-ukk" title="UKK" subtitle="Usein kysytyt kysymykset ennen projektin aloitusta." dark />
      <section className="bg-[#111] px-5 pb-12"><div className="grid gap-5">{faqs.map((faq, index) => { const isOpen = openFaq === index; return <DarkCard key={faq.question} className="overflow-hidden"><button onClick={() => setOpenFaq(isOpen ? -1 : index)} className="flex w-full items-center justify-between gap-4 p-6 text-left"><span className="font-serif text-[29px] leading-tight text-white">{faq.question}</span><ChevronDown className={`h-6 w-6 shrink-0 text-[#f28a12] ${isOpen ? "rotate-180" : ""}`} /></button>{isOpen && <p className="border-t border-white/10 px-6 pb-6 pt-4 text-[17px] leading-8 text-white/65">{faq.answer}</p>}</DarkCard>; })}</div></section>

      <SectionIntro id="m-yhteys" title="Yhteys" subtitle="Kerro lyhyesti yrityksestäsi ja millaiset sivut tarvitset." />
      <section className="bg-[#f28a12] px-5 pb-14"><DarkCard className="p-6">{contactItems.map((item) => { const RowIcon = item.icon; return <div key={item.title} className="flex gap-5 border-b border-[#f28a12]/30 py-6 first:pt-0 last:border-0 last:pb-0"><RowIcon className="h-14 w-14 shrink-0 rounded-full border border-[#f28a12] p-3 text-[#f28a12]" /><div><h3 className="font-serif text-2xl text-white">{item.title}</h3><p className="mt-2 leading-7 text-white/62">{item.text}</p></div></div>; })}</DarkCard><DarkCard className="mt-7 p-6"><form onSubmit={(event) => event.preventDefault()} className="grid gap-4">{["Nimi", "Yritys", "Sähköposti", "Puhelin"].map((field) => <input key={field} aria-label={field} placeholder={field} className="h-14 w-full rounded-xl border border-[#f28a12]/25 bg-black/20 px-5 text-white outline-none placeholder:text-white/45" />)}<textarea aria-label="Viesti" placeholder="Viesti" rows={7} className="w-full resize-none rounded-xl border border-[#f28a12]/25 bg-black/20 px-5 py-4 text-white outline-none placeholder:text-white/45" /><button className="flex min-h-[64px] w-full items-center justify-center rounded-xl bg-[#f28a12] px-6 text-lg font-medium text-[#15120d]">Lähetä viesti <ArrowRight className="ml-3 h-5 w-5" /></button><p className="flex items-center justify-center gap-2 text-sm text-white/45"><LockKeyhole className="h-4 w-4" />Tietosi käsitellään luottamuksellisesti.</p></form></DarkCard></section>

      <footer className="bg-[linear-gradient(90deg,#f28a12_0%,#f28a12_50%,#1d1d1d_50%,#1d1d1d_100%)] px-5 py-14"><DarkCard className="p-7"><div className="text-3xl font-light tracking-[0.36em] text-white">NODRA</div><h2 className="mt-10 font-serif text-[40px] leading-[1.05] tracking-[-0.05em] text-white">Selkeät verkkosivut pienille yrityksille.</h2><div className="mt-6 h-[2px] w-20 bg-[#f28a12]" /><p className="mt-7 text-[17px] leading-8 text-white/65">Moderneja ja mobiiliystävällisiä verkkosivuja yrityksille, jotka haluavat näyttää luotettavilta ja tehdä yhteydenotosta helppoa.</p><div className="mt-8 flex gap-6 text-[#f28a12]"><Linkedin /><Instagram /><Mail /></div><div className="mt-10 border-t border-white/10 pt-8"><h3 className="text-xl text-[#f28a12]">Pikavalikot</h3>{navItems.map((item) => <button key={item.id} onClick={() => goTo(item.id)} className="flex w-full items-center justify-between border-b border-white/10 py-4 text-left text-lg text-white/82">{item.label}<ArrowRight className="h-5 w-5 text-[#f28a12]" /></button>)}</div><div className="mt-10 border-t border-white/10 pt-8"><h3 className="text-xl text-[#f28a12]">Ota yhteyttä</h3><div className="mt-6 grid gap-6"><p className="flex gap-4"><Mail className="text-[#f28a12]" /> {emailAddress}</p><p className="flex gap-4"><MapPin className="text-[#f28a12]" /> Etänä koko Suomeen</p></div></div><p className="mt-10 border-t border-white/10 pt-6 text-sm text-white/50">© NODRA 2026 · Kaikki oikeudet pidätetään.</p></DarkCard></footer>
    </main>
  );
}
